<template>
  <HomePanel ref="target" title="最新专题">
    <template v-slot:right>
      <XtxMore />
    </template>
    <template v-slot:default>
      <div class="special-list">
        <template v-if="homeSpecialList">
          <div
            v-for="item in homeSpecialList"
            :key="item.id"
            class="special-item"
          >
            <RouterLink to="/">
...
import { getHomeSpecialApi } from "../../../api/home";
...
export default {
  ...
  setup() {
    const { target, result: homeSpecialList } = useLazyData(getHomeSpecialApi);
    return { target, homeSpecialList };
  },
};
</script>
